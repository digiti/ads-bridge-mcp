...
from ..normalize import micros_to_display, safe_divide
...
async def get_pacing_report(
    ...
    for idx, account_id in enumerate(meta_account_ids):
        ...
        spent_micros = sum(
            int(float(row.get("spend", 0) or 0) * 1_000_000) for row in insights if isinstance(row, dict)
        )
